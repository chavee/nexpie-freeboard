...
(function()
{
    freeboard.loadDatasourcePlugin({
        "type_name"   : "nexpie_feed",
        "display_name": "NEXPIE Feed",
        "description" : "NEXPIE Feed Datasource.",
        "external_scripts" : [
...
*/
        ...
        newInstance : function(settings, newInstanceCallback, updateCallback) {
            newInstanceCallback(new feedNexpieDatasourcePlugin(settings, updateCallback));
        }
    });


    var feedNexpieDatasourcePlugin = function(settings, updateCallback) {
        var self = this;
        ...
        function reloadhData(s) {
            if(typeof data['data'] !== "undefined"){
                ...
                var apiurl = 'http://alpha.nexpie.io:9876/feed/'+s.feedid+'?granularity='+s.granularity_value+s.granularity_unit+'&aggregate=avg&from='+lasttime+'&to='+timenow;
                ...
                var apiurl = 'http://alpha.nexpie.io:9876/feed/'+s.feedid+'?granularity='+s.granularity_value+s.granularity_unit+'&aggregate=avg&since='+s.since_value+s.since_unit;
                console.log(apiurl)
                $.getJSON( apiurl, function(datajson) {
                    data['data'] = datajson;
                    var newfrom;
        ...
        function reloadiData(s) {
            if(typeof data['data'] !== "undefined"){
                ...
                if(timelastitem+s.granularity_value*timelist[s.granularity_unit]<s.to_value){
                    var apiurl = 'http://alpha.nexpie.io:9876/feed/'+s.feedid+'?granularity='+s.granularity_value+s.granularity_unit+'&aggregate=avg&from='+timelastitem+'&to='+s.to_value;
                    $.getJSON( apiurl, function(datajson) {
                        if(typeof datajson['lastest_data'] !== "undefined"){
                            for (var i = 0; i<data['data']['lastest_data'].length ; i++) {
                                for (var j = 0; j<datajson['data'].length ; j++) {
                ...
                var apiurl = 'http://alpha.nexpie.io:9876/feed/'+s.feedid+'?granularity='+s.granularity_value+s.granularity_unit+'&aggregate=avg&from='+s.from_value+'&to='+s.to_value;
                $.getJSON( apiurl, function(datajson) {
                    data['data'] = datajson;
                    var newfrom;
                    var newto;
        ...
        function reloadjData(s,from,to) {
            var apiurl = 'http://alpha.nexpie.io:9876/feed/'+s.feedid+'?apikey='+s.apikey+'&granularity='+s.granularity_value+s.granularity_unit+'&aggregate=avg&from='+from+'&to='+to;
            $.getJSON( apiurl, function(datajson) {
                for (var i = 0; i<data['data']['lastest_data'].length ; i++) {
